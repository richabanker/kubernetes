    ...
    if review_comment:
        ...
        for line_num, comment in line_comments:
            ...
            try:
                ...
                right_side_line = 0
                current_line = 0
                added_lines = 0 # Track added lines to calculate offset.

                for diff_line in diff_lines:
                    if diff_line.startswith("@@"):
                        # Extract right-side line number and added lines from hunk info
                        ...
                        added_lines = 0 # Reset when a new hunk starts

                    elif diff_line.startswith("+"):
                        current_line += 1
                        added_lines +=1 #increment added lines.
                        if current_line == line_num:
                            corrected_line_num = current_line
                            break

                    elif not diff_line.startswith("-"): # count unchanged lines.
                        current_line += 1
                ...
                if corrected_line_num:
                    ...
                    print(f"WARNING: Could not find line {line_num} in {diff_file.filename}. Gemini may be providing incorrect line numbers, or the line was removed.")
                    print(f"Gemini comment: {comment}") #print the comment, for debugging.
                    print(f"Diff file: {diff_file.filename}") #print the file name.
