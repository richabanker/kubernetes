...
func TestCacheWatcherDrainingNoBookmarkAfterResourceVersionSent(t *testing.T) {
	...
	if !w.add(&watchCacheEvent{Object: makePod(5), ResourceVersion: 5, RecordTime: fakeClock.Now().Add(-2 * time.Second), timeline: metrics.DispatchTimeline{metrics.PointCacheReceived: fakeClock.Now().Add(-1 * time.Second)}}, time.NewTimer(1*time.Second)) {
		t.Fatal("failed adding an even to the watcher")
	}
	...
	if !w.add(&watchCacheEvent{Object: makePod(15), ResourceVersion: 15, RecordTime: fakeClock.Now().Add(-2 * time.Second), timeline: metrics.DispatchTimeline{metrics.PointCacheReceived: fakeClock.Now().Add(-1 * time.Second)}}, time.NewTimer(1*time.Second)) {
		t.Fatal("failed adding an even to the watcher")
	}
