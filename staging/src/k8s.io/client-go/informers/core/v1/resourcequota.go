...
type ResourceQuotaInformerOptions struct {
	...
	// The full informer name will be InformerNamePrefix + "-resourceQuota-informer",
...
func NewResourceQuotaInformer(client kubernetes.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
	return NewResourceQuotaInformerWithOptions(client, namespace, ResourceQuotaInformerOptions{ResyncPeriod: resyncPeriod, Indexers: indexers})
}
...
func NewFilteredResourceQuotaInformer(client kubernetes.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
	return NewResourceQuotaInformerWithOptions(client, namespace, ResourceQuotaInformerOptions{ResyncPeriod: resyncPeriod, Indexers: indexers, TweakListOptions: tweakListOptions})
}

// NewResourceQuotaInformerWithOptions constructs a new informer for ResourceQuota type with additional options.
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewResourceQuotaInformerWithOptions(client kubernetes.Interface, namespace string, options ResourceQuotaInformerOptions) cache.SharedIndexInformer {
	...
	if options.InformerNamePrefix != "" {
		...
		identifier, _ = cache.NewIdentifier(options.InformerNamePrefix+"-resourceQuota-informer", gvr)
	}
...
func (f *resourceQuotaInformer) defaultInformer(client kubernetes.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
	return NewResourceQuotaInformerWithOptions(client, f.namespace, ResourceQuotaInformerOptions{ResyncPeriod: resyncPeriod, Indexers: cache.Indexers{cache.NamespaceIndex: cache.MetaNamespaceIndexFunc}, InformerNamePrefix: f.factory.InformerNamePrefix(), TweakListOptions: f.tweakListOptions})
}
