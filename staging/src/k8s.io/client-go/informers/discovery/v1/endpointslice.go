...
type EndpointSliceInformerOptions struct {
	...
	// The full informer name will be InformerNamePrefix + "-endpointSlice-informer",
...
func NewEndpointSliceInformer(client kubernetes.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
	return NewEndpointSliceInformerWithOptions(client, namespace, EndpointSliceInformerOptions{ResyncPeriod: resyncPeriod, Indexers: indexers})
}
...
func NewFilteredEndpointSliceInformer(client kubernetes.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
	return NewEndpointSliceInformerWithOptions(client, namespace, EndpointSliceInformerOptions{ResyncPeriod: resyncPeriod, Indexers: indexers, TweakListOptions: tweakListOptions})
}

// NewEndpointSliceInformerWithOptions constructs a new informer for EndpointSlice type with additional options.
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewEndpointSliceInformerWithOptions(client kubernetes.Interface, namespace string, options EndpointSliceInformerOptions) cache.SharedIndexInformer {
	...
	if options.InformerNamePrefix != "" {
		...
		identifier, _ = cache.NewIdentifier(options.InformerNamePrefix+"-endpointSlice-informer", gvr)
	}
...
func (f *endpointSliceInformer) defaultInformer(client kubernetes.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
	return NewEndpointSliceInformerWithOptions(client, f.namespace, EndpointSliceInformerOptions{ResyncPeriod: resyncPeriod, Indexers: cache.Indexers{cache.NamespaceIndex: cache.MetaNamespaceIndexFunc}, InformerNamePrefix: f.factory.InformerNamePrefix(), TweakListOptions: f.tweakListOptions})
}
